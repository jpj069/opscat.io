...
function mkCase(severity = 85, orgId = DEFAULT_ORG_ID, name = 'disk_full') {
  return Number(db.prepare(`INSERT INTO cases (org_id, event_id, name, device, severity, status, opened_at)
    VALUES (?, NULL, ?, 'core-01', ?, 'open', ?)`).run(orgId, name, severity, now()).lastInsertRowid);
}
...
const attemptsOf = (id) => db.prepare('SELECT * FROM alert_attempts WHERE alert_id = ? ORDER BY id').all(id);
const timersOf = (id) => db.prepare('SELECT * FROM alert_timers WHERE alert_id = ? AND canceled_at IS NULL').all(id);
...
async function main() {
  ...
  // ---- migration ---------------------------------------------------------
  chk('migration reached version 24', db.pragma('user_version', { simple: true }) >= 24);
  ...
  const alertDdl = db.prepare("SELECT sql FROM sqlite_master WHERE name='alerts'").get().sql;
  chk('alerts.org_id is TEXT (uuid), not INTEGER', /org_id\s+TEXT/.test(alertDdl));
  chk('alerts.acked_by is TEXT (uuid)', /acked_by\s+TEXT/.test(alertDdl));
  const tgtDdl = db.prepare("SELECT sql FROM sqlite_master WHERE name='escalation_targets'").get().sql;
  // One column holds a uuid (user) and an integer (schedule/team) — it has to be
  // the wider type, or every user target silently stops matching.
  chk('escalation_targets.ref_id is TEXT — it holds uuids AND integer ids', /ref_id\s+TEXT/.test(tgtDdl));
  chk('cases gained acked_at/acked_by', db.pragma('table_info(cases)').some((c) => c.name === 'acked_at')
    && db.pragma('table_info(cases)').some((c) => c.name === 'acked_by'));
  chk('notifications gained alert_id', db.pragma('table_info(notifications)').some((c) => c.name === 'alert_id'));
  chk('alert_rules gained target_type defaulting to "channel"',
    db.pragma('table_info(alert_rules)').some((c) => c.name === 'target_type' && /channel/.test(String(c.dflt_value))));
  ...
  chk('...but the number is never in the clear',
    db.prepare("SELECT COUNT(*) c FROM contact_methods WHERE kind='sms' AND encrypted = 0").get().c === 0);
  chk('...and it may not be rung until it is verified',
    require('./src/lib/contacts').usable(
      db.prepare("SELECT * FROM contact_methods WHERE kind='sms'").get(), DEFAULT_ORG_ID).ok === false);
  ...
  chk('a step timer is armed', timersOf(A.id).some((t) => t.kind === 'step'));
  ...
  chk('the delivery is recorded in the ONE notification log',
    notifsFor(A.id).some((n) => n.ok === 1 && n.channel === 'ntfy'));
  ...
  chk('and did not notify a second time', attemptsOf(A.id).length === 1);
  ...
  await tick();
  chk('step 0 rings the on-call person', attemptsOf(B.id).length === 1);

  chain.sweep(now() + 4 * MIN);
  chk('a sweep BEFORE the timeout changes nothing', alertRow(B.id).step_position === 0);

  chain.sweep(now() + 6 * MIN);
  await tick();
  const b1 = alertRow(B.id);
  ...
  chk('rung two reaches the NEXT target, exactly once',
    attemptsOf(B.id).length === 2 && attemptsOf(B.id)[1].user_id === ben.id);
  chk('and it went to ben\'s own contact method', delivered.some((d) => d.url === '/ben'));

  chain.sweep(now() + 12 * MIN);
  await tick();
  const b2 = alertRow(B.id);
  ...
  chain.sweep(now() + 18 * MIN);
  chain.sweep(now() + 24 * MIN);
  await tick();
  const b3 = alertRow(B.id);
  ...
  const exhausted = eventsNamed('alert_exhausted');
  ...
  chk('every rung is in the attempt record', attemptsOf(B.id).length === 4,
    `${attemptsOf(B.id).length} attempts`);
  chk('an exhausted alert has no timers left', timersOf(B.id).length === 0);
  ...
  chk('closing the case through PATCH cancels the alert', alertRow(C.id).status === 'canceled',
    alertRow(C.id).status);
  chk('and drops its timers — nobody is woken about a problem that is over',
    timersOf(C.id).length === 0);

  // The second door: finishing the EVENT, which closes the case underneath it.
  pipeline.ingestEvent({ name: 'disk_full', device: 'core-02', target: null, severity: 90,
    description: 'disk full on core-02' }, 'e2e', false, DEFAULT_ORG_ID);
  const ev = db.prepare("SELECT * FROM events WHERE device = 'core-02' ORDER BY id DESC LIMIT 1").get();
  const caseD = db.prepare('SELECT * FROM cases WHERE event_id = ?').get(ev.id);
  ...
  chk('finishing the EVENT cancels the alert too (the other seven close sites)',
    alertRow(D.id).status === 'canceled', alertRow(D.id).status);

  // The third door: an incident resolving.
  const incId = Number(db.prepare(`INSERT INTO incidents (org_id, title, severity, status, started_at, created_by)
    VALUES (?, 'Datacentre power', 90, 'investigating', ?, ?)`).run(DEFAULT_ORG_ID, now(), anna.id).lastInsertRowid);
  ...
  chk('an alert may hang on an INCIDENT, not only a case (§13.5)', E && E.subjectKind === 'incident',
    JSON.stringify(E));
  require('./src/lib/incidents').setStatus(DEFAULT_ORG_ID, anna.id, incId, 'resolved', 'power back');
  chk('resolving the incident cancels its alert', alertRow(E.id).status === 'canceled');
  ...
  chk('an acknowledged alert whose case closes ends as RESOLVED, not canceled',
    alertRow(F.id).status === 'resolved', alertRow(F.id).status);

  // ---- acknowledging from the case ----------------------------------------
  const caseG = mkCase(90);
  ...
  chk('acknowledging the CASE acknowledges what is ringing about it',
    alertRow(G.id).status === 'acked', alertRow(G.id).status);
  ...
  chain.sweep(now() + 60 * MIN);
  const h = alertRow(H.id);
  ...
  chk('...and has no clock running at all', timersOf(H.id).length === 0);
  ...
  chk('...and the suppression is VISIBLE in the notification log, not silent',
    !!db.prepare("SELECT 1 x FROM notifications WHERE error LIKE '%outside support hours%'").get());
  const caseJ = mkCase(95);
  ...
  const winId = db.prepare(`INSERT INTO maintenance_windows (org_id, name, starts_at, ends_at, created_at)
    VALUES (?, 'Planned patching', ?, ?, ?)`).run(DEFAULT_ORG_ID, now() - MIN, now() + 60 * MIN, now()).lastInsertRowid;
  const caseK = mkCase(95);
  ...
  chk('...with the same visible suppression line a channel rule writes',
    !!db.prepare("SELECT 1 x FROM notifications WHERE error LIKE '%maintenance window%'").get());
  db.prepare('DELETE FROM maintenance_windows WHERE id = ?').run(winId);
  ...
  chk('...and it reached the next rung\'s target', attemptsOf(L.id).some((a) => a.user_id === ben.id));
  chk('the gap itself is reported as an event', eventsNamed('oncall_gap').length >= 1);
  chk('...and the reason is in the notification log',
    notifsFor(L.id).some((n) => /escalating now/.test(n.error || '')));
  ...
  chk('a channel that refuses is recorded as a FAILED delivery',
    notifsFor(M.id).some((n) => n.ok === 0 && /500/.test(n.error || '')), JSON.stringify(notifsFor(M.id)));
  chk('a system that cannot reach anybody raises alert_undeliverable — it must not look like a quiet night',
    eventsNamed('alert_undeliverable').length === 1, `${eventsNamed('alert_undeliverable').length}`);

  // ---- the e-mail fallback ------------------------------------------------
  const dave = mkUser('dave@e2e.test', 'analyst');
  const davePol = (await mkPolicy({
    name: 'Fallback', steps: [{ timeoutM: 5, targets: [{ kind: 'user', refId: dave.id }] }],
  })).j;
  const caseN = mkCase(90);
  ...
  chk('a person with no contact method is still tried, on their account e-mail',
    notifsFor(N.id).some((n) => n.channel === 'email'), JSON.stringify(notifsFor(N.id)));
  ...
  chk('a rule that could not fire inside the step is SKIPPED and says so',
    notifsFor(O.id).some((n) => /exceeds|escalates after/.test(n.error || '')),
    JSON.stringify(notifsFor(O.id).map((n) => n.error)));
  chk('a delayed rule that fits inside the step becomes a timer',
    timersOf(O.id).some((t) => t.kind === 'notify'));
  chain.sweep(now() + 3 * MIN);
  await tick();
  ...
  delivered.length = 0;
  pipeline.ingestEvent({ name: 'db_down', device: 'db-01', target: null, severity: 95,
    description: 'primary database unreachable' }, 'e2e', false, DEFAULT_ORG_ID);
  await tick(); await tick();
  const fromRule = db.prepare("SELECT * FROM alerts WHERE source LIKE 'rule:%' ORDER BY id DESC LIMIT 1").get();
  ...
  chk('and the on-call person was rung', fromRule && attemptsOf(fromRule.id).length === 1);
  ...
  const alertsBefore = db.prepare('SELECT COUNT(*) c FROM alerts').get().c;
  pipeline.ingestEvent({ name: 'alert_exhausted', device: 'C-1', target: null, severity: 85,
    description: 'x' }, 'e2e', false, DEFAULT_ORG_ID);
  await tick();
  chk('a policy rule on the chain\'s OWN signal is refused — no perpetual motion',
    db.prepare('SELECT COUNT(*) c FROM alerts').get().c === alertsBefore);
  chk('...and the refusal is recorded, not silent',
    !!db.prepare("SELECT 1 x FROM notifications WHERE error LIKE '%raised BY the alert chain%'").get());
  ...
  chk('the escalation clock is a ROW, not a setTimeout',
    db.prepare("SELECT COUNT(*) c FROM alert_timers WHERE alert_id = ? AND kind = 'step'").get(Q.id).c === 1);
  // Fall due in the past, as if the process had been down through the timeout.
  db.prepare('UPDATE alert_timers SET resume_at = ? WHERE alert_id = ?').run(now() - MIN, Q.id);
  ...
  chk('...and counts only cases that were ACTUALLY acknowledged',
    ana.mtta.acked === db.prepare('SELECT COUNT(*) c FROM cases WHERE acked_at IS NOT NULL').get().c,
    `${ana.mtta.acked} vs ${db.prepare('SELECT COUNT(*) c FROM cases WHERE acked_at IS NOT NULL').get().c}`);
  ...
  const tzUser = mkUser('night@e2e.test', 'analyst');
  db.prepare('UPDATE users SET timezone = ? WHERE id = ?').run('Europe/Berlin', tzUser.id);
  ...
  const anyAlert = db.prepare('SELECT id FROM alerts ORDER BY id LIMIT 1').get().id;
  for (const at of [nightAt, dayAt]) {
    db.prepare(`INSERT INTO alert_attempts (alert_id, user_id, step_position, round, via, notified_at)
  ...
  require('./src/engine/retention').prune();
  chk('an ENDED alert older than the window is pruned; its attempts cascade',
    (() => {
      const old = Number(db.prepare(`INSERT INTO alerts (org_id, subject_kind, subject_id, urgency,
        status, source, created_at, ended_at, end_reason)
        VALUES (?, 'case', 999, 'high', 'canceled', 'test', ?, ?, 'old')`)
        .run(DEFAULT_ORG_ID, now() - 400 * 86400000, now() - 400 * 86400000).lastInsertRowid);
      db.prepare(`INSERT INTO alert_attempts (alert_id, user_id, step_position, round, via, notified_at)
        VALUES (?, ?, 0, 0, 'user', ?)`).run(old, anna.id, now() - 400 * 86400000);
      require('./src/engine/retention').prune();
      return !alertRow(old)
        && db.prepare('SELECT COUNT(*) c FROM alert_attempts WHERE alert_id = ?').get(old).c === 0;
    })());
  chk('an ACTIVE alert is never pruned, however old — it is still ringing',
    (() => {
      const live = Number(db.prepare(`INSERT INTO alerts (org_id, subject_kind, subject_id, urgency,
        status, source, created_at) VALUES (?, 'case', 998, 'high', 'active', 'test', ?)`)
        .run(DEFAULT_ORG_ID, now() - 400 * 86400000).lastInsertRowid);
      require('./src/engine/retention').prune();
      return !!alertRow(live);
    })());
  ...
  chk('its steps go with it',
    db.prepare('SELECT COUNT(*) c FROM escalation_steps WHERE policy_id = ?').get(deadPol.id).c === 0);
  chk('an alert that outlived its policy keeps its history rather than vanishing',
    !!alertRow(M.id), 'alert row disappeared');
