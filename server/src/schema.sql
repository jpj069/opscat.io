...
CREATE TABLE IF NOT EXISTS sessions (
  ...
  ip            TEXT,
  user_agent    TEXT,
  -- Set only on a session minted by superadmin impersonation: the operator who
  -- is really driving it. Drives the banner and the way back (routes/auth.js
  -- § stop-impersonating). SET NULL on delete — losing the operator's account
  -- must not delete the customer sessions they opened, it must only close the
  -- door home.
...
CREATE TABLE IF NOT EXISTS syslog_endpoints (
  ...
  -- 'collector' (the customer runs one in their network) or 'managed' (they
  -- send straight to our gateway). Both use the same key on the same ingest
  -- endpoint; what differs is the configuration the product prints.
  mode          TEXT NOT NULL DEFAULT 'collector',
  -- Tunnel mode only (see migration 031). The peer's WireGuard public key, and
  -- the inner address we allocated to it. Inside the tunnel the address IS the
  -- identity — the kernel will not carry a packet from an address the peer's
  -- key does not own — so plain UDP becomes attributable and appliances that
  -- cannot carry a credential become reachable.
  peer_pubkey   TEXT,
  -- An inner address belongs to exactly one endpoint, across every
  -- organisation. That is the whole tenant boundary of the tunnel path, and it
  -- is a column constraint HERE rather than a `CREATE UNIQUE INDEX` below —
  -- which is what took production down for 23 minutes on 2026-08-24.
  --
  -- This file is applied at every boot, BEFORE migrations. On an existing
  -- database `CREATE TABLE IF NOT EXISTS` is a no-op, so the columns the newest
  -- migration adds are not there yet — and a standalone statement referencing
  -- one of them dies with `column "tunnel_ip" does not exist`, in applySchema,
  -- before the migration that would have added it can run. Boot loop, on every
  -- install that already existed, from a commit whose fresh-install path was
  -- perfect.
  --
  -- `UNIQUE` is equivalent to the partial index migration 031 creates: Postgres
  -- treats NULLs as distinct, so many endpoints may have no address.
...
CREATE TABLE IF NOT EXISTS agents (
  id            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  org_id        UUID NOT NULL DEFAULT '00000000-0000-4000-8000-000000000001',
  -- Unique WITHIN an org, never globally. A name is a label its own tenant
  -- picks, and a global constraint made the first org to use one own it for
  -- everybody: onboarding registers `my-first-server`, so exactly ONE
  -- organisation per instance could ever finish that step and every other one
  -- was refused with a 409 naming a row it cannot see. (migration 036)
  name          TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS synthetic_locations (
  ...
  active        BIGINT NOT NULL DEFAULT 1,
  last_seen_at  BIGINT,
  last_ip       TEXT,                        -- source address of the last work-list pull
  created_at    BIGINT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS sensor_nodes (
  ...
  agent_version TEXT,
  agent_id      BIGINT REFERENCES agents(id) ON DELETE SET NULL,  -- host-agent registration
...
-- Where a credential has EVER launched an instance. The orphan sweeper
-- (engine/reconcile.js) reads its region list from HERE and not from
-- `sensor_nodes`, because that is the table it deletes from: once the last node
-- in a region was gone, the region stopped being listed and a leaked instance
-- there billed forever. Rows are written on a successful launch and never
...
CREATE TABLE IF NOT EXISTS synthetic_checks (
  ...
  user_agent    TEXT,                        -- http only; NULL = org default, then OpsCat's own
  created_at    BIGINT NOT NULL
);
...
-- The organisation's own avatar. An org had a NAME and nothing else, so every
-- place that wanted to show one drew a placeholder instead: the workspace
-- switcher had a fixed indigo gradient square, identical for every org, and the
-- alert mail grew a blue square that was decoration pretending to be identity.
--
-- This is deliberately NOT `status_assets`. That table hangs off a status PAGE
-- and wears that page's brand — a customer may run several pages with different
-- logos, and none of them is the organisation. Same storage shape, different
-- owner, so the sniffing and the size cap are shared (lib/status-branding.js)
-- and the row is not.
--
-- There is no row for the DEFAULT: initials over a colour derived from the org
-- id (lib/org-avatar.js) need no storage and are therefore never missing.
CREATE TABLE IF NOT EXISTS org_assets (
  org_id     UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  kind       TEXT NOT NULL CHECK (kind IN ('avatar')),
  mime       TEXT NOT NULL,
  bytes      BYTEA NOT NULL,
  updated_at BIGINT NOT NULL,         -- drives the ETag and the cache-busting v=
...
CREATE TABLE IF NOT EXISTS oauth_codes (
  ...
  -- Which organizations this grant covers. `org_id` above stays the PRIMARY one
  -- (it is what revocation and the grant list key on, and what a pre-multi-org
  -- token carries alone). `org_scope` is 'list' — exactly the ids in `org_ids` —
  -- or 'all', meaning every org the user is a member of, resolved per request.
  -- Neither is authoritative on its own: `memberships` is, and the stored set is
  -- only an upper bound. A `list` grant with `org_ids` NULL means [org_id].
  org_scope      TEXT NOT NULL DEFAULT 'list',
  org_ids        TEXT,                       -- csv of uuids; NULL when scope = 'all'
...
CREATE TABLE IF NOT EXISTS oauth_tokens (
  ...
  org_id        UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  scopes        TEXT NOT NULL,
  org_scope     TEXT NOT NULL DEFAULT 'list',  -- 'list' = org_ids, 'all' = every membership
  org_ids       TEXT,                          -- csv of uuids; NULL when scope = 'all'
