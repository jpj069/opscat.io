 ...
 *
 * UUIDv4, generated by us rather than by the database — SQLite has no UUID type
 * and no generator, so `lastInsertRowid` (which only exists for INTEGER keys) is
 * replaced by "mint the id, then insert it". That also means an insert knows its
 * own id before it runs, which several call sites needed anyway.
 *
 * Stored as TEXT: 36 bytes against BLOB's 16, in exchange for a database anyone
 * can read with `sqlite3` and a value that survives a JSON round trip untouched.
 */
