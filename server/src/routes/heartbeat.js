...
const findByToken = q.prepare('SELECT id FROM heartbeats WHERE token_hash = ? AND enabled = 1');
const ping = q.prepare('UPDATE heartbeats SET last_ping_at = ?, alerted_at = NULL WHERE id = ?');

router.all('/heartbeat/:token', async (req, res) => {
  if (req.method !== 'GET' && req.method !== 'POST' && req.method !== 'HEAD') {
    return httpError(res, 405, 'use GET or POST');
  }
  const row = await findByToken.get(sha256(String(req.params.token || '')));
  if (!row) return httpError(res, 404, 'unknown heartbeat');
  await ping.run(now(), row.id);
  res.json({ ok: true });
});
