...
const { now, sha256, isStr, optStr, isEmail, clampInt, httpError, isId, utcDaySql, utcDayLabel } = require('../util');
...
router.get('/teams', async (req, res) => res.json(
  await Promise.all((await qTeams.all(req.orgId)).map(teamView))));
...
router.post('/teams', sec.requireRole('lead'), async (req, res) => {
  ...
  res.status(201).json(await teamView(await qTeam.get(id, req.orgId)));
});

router.patch('/teams/:id', sec.requireRole('lead'), async (req, res) => {
  ...
  res.json(await teamView(await qTeam.get(t.id, req.orgId)));
});

router.delete('/teams/:id', sec.requireRole('lead'), async (req, res) => {
  const t = await qTeam.get(req.params.id, req.orgId);
  if (!t) return httpError(res, 404, 'team not found');
...
router.get('/schedules', async (req, res) => {
  const at = now();
  res.json(await Promise.all((await qSchedules.all(req.orgId)).map((s) => scheduleView(s, at))));
});

router.get('/schedules/:id', async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
  res.json(await scheduleView(s));
});

router.post('/schedules', sec.requireRole('lead'), async (req, res) => {
  ...
  res.status(201).json(await scheduleView(await qSchedule.get(id, req.orgId)));
});

router.patch('/schedules/:id', sec.requireRole('lead'), async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
  const { name, timezone, teamId } = req.body || {};
  ...
  res.json(await scheduleView(await qSchedule.get(s.id, req.orgId)));
});

router.delete('/schedules/:id', sec.requireRole('lead'), async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
...
router.put('/schedules/:id/layers', sec.requireRole('lead'), async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
  ...
  for (const [i, l] of layers.entries()) {
    ...
    for (const uid of l.participants) {
      if (!(await isMember(uid, req.orgId))) return httpError(res, 400, 'unknown participant');
    }
    let restrict = null;
    if (l.restrict) {
      const r = l.restrict;
...
router.post('/schedules/:id/overrides', sec.requireRole('lead'), async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
  const { userId, startsAt, endsAt } = req.body || {};
  if (!(await isMember(userId, req.orgId))) return httpError(res, 400, 'unknown user');
  ...
  res.status(201).json({ id, ...(await scheduleView(await qSchedule.get(s.id, req.orgId))) });
});

router.delete('/overrides/:id', sec.requireRole('lead'), async (req, res) => {
...
router.get('/policies', async (req, res) => res.json(
  await Promise.all((await qPolicies.all(req.orgId)).map(policyView))));

router.post('/policies', sec.requireRole('lead'), async (req, res) => {
  ...
  res.status(201).json(await policyView(await qPolicy.get(id, req.orgId)));
});

router.patch('/policies/:id', sec.requireRole('lead'), async (req, res) => {
  ...
  res.json(await policyView(await qPolicy.get(p.id, req.orgId)));
});

router.delete('/policies/:id', sec.requireRole('lead'), async (req, res) => {
  const p = await qPolicy.get(req.params.id, req.orgId);
  if (!p) return httpError(res, 404, 'policy not found');
...
router.get('/alerts', async (req, res) => {
  ...
  res.json(await Promise.all((await qAlerts.all(req.orgId, filter, filter, filter, limit))
    .map((a) => chain.view(a))));
});
...
router.get('/alerts/:id', async (req, res) => {
  ...
  res.json(await chain.view(a, { includeAttempts: true }));
});
...
router.post('/alerts', async (req, res) => {
  ...
  res.status(r.already ? 200 : 201).json(await chain.view(r.alert, { includeAttempts: true }));
});
...
router.post('/alerts/:id/ack', async (req, res) => {
  ...
  res.json(await chain.view(r.alert, { includeAttempts: true }));
});

router.post('/alerts/:id/cancel', async (req, res) => {
  const a = await qAlert.get(req.params.id, req.orgId);
  if (!a) return httpError(res, 404, 'alert not found');
  ...
  res.json(await chain.view(await qAlert.get(a.id, req.orgId)));
});
...
router.post('/me/methods', async (req, res) => {
...
router.post('/me/methods/:id/verify/confirm', async (req, res) => {
  const m = await qMethod.get(req.params.id, req.user.id);
  if (!m) return httpError(res, 404, 'method not found');
...
router.get('/me/push-key', async (req, res) => res.json({ key: await webpush.publicKey() }));
...
router.delete('/me/methods/:id', async (req, res) => {
  const m = await qMethod.get(req.params.id, req.user.id);
  if (!m) return httpError(res, 404, 'method not found');
...
router.put('/me/rules', async (req, res) => {
  ...
  for (const r of raw) {
    ...
    if (!(await qMethod.get(Number(r.methodId), req.user.id))) return httpError(res, 400, 'unknown contact method');
...
router.get('/now', async (req, res) => res.json({ at: now(), schedules: await oncall.onCallNow(req.orgId) }));

router.get('/schedules/:id/timeline', async (req, res) => {
  const s = await qSchedule.get(req.params.id, req.orgId);
  if (!s) return httpError(res, 404, 'schedule not found');
