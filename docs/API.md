...
The consent screen carries **organization checkboxes** (migration 037): a connection
covers a SET of organizations — all pre-ticked, plus an opt-in for the ones joined later
(`org_scope` `list`|`all`). A connection covering one organization is unchanged and takes
no org argument; covering several, **every tool takes a required `organization`** (the
name — `opscat_list_organizations` reports the valid ones) and there is no default: a call
naming none is refused rather than answered for whichever org sorts first. The stored set
is an upper bound only — it is intersected with `memberships` on every request, so leaving
an organization removes it from a live connection without touching the other ones, and the
role is re-checked per call against the organization that call resolved to. Roles are
read from `memberships` per request, never cached in the token. 21 tools (11 read, 10 write) — including `opscat_who_is_oncall`, which names any schedule that resolves to NOBODY separately in `gaps` so an agent does not have to notice a null in a list, plus `opscat_list_alerts` and `opscat_ack_alert` — each annotated and carrying an `outputSchema`; a read-only
...
- **Status pages** (`lib/status-pages.js`, `lib/status-branding.js`, `lib/status-domains.js`, schema v18). A status page is a ROW, not an implicit per-org thing: an org has a default page (slug = the org's slug, so every pre-v18 URL still resolves) and, on Enterprise, additional ones. A page is addressable, most specific first: its own **verified domain** → the dedicated **status host** when `OPSCAT_STATUS_HOST` is set (`status.<domain>/<slug>`, every page by slug including the default one; a bare root there redirects to the origin org's page) → its **slug** (`/status/<slug>`) → the origin default (`/status`). Setting the status host does not retire the `/status` paths — a status page URL ends up in runbooks. Slugs that would shadow an application path on that host (`api`, `app`, `v1`, `assets`, …) are reserved. Worth being honest about the trade: the usual argument for a separate status host is surviving the app being down, and that only holds for separate INFRASTRUCTURE — here it is the same process, so this is naming, not resilience. **Pricing shape** (see `plans.js`): the page's IDENTITY — logo, favicon, accent, light/dark, description, support/legal links — is **ungated on every plan including Free**, because a status page is public and a Free one wearing a generic OpsCat mark advertises us badly rather than pressuring an upgrade. What is paid for is distance from OpsCat and what costs us to run: `status_domain` (pro+), `status_whitelabel` + `status_css` (business+), `status_pages_multi` (enterprise). All four are refused on WRITE with a 403 (a switch that silently does nothing is worse than one that explains itself) **and** re-checked on RENDER, so a downgrade stops the behaviour without destroying the configuration — an upgrade restores it untouched. Admin (read lead+, write admin): `GET /api/admin/status-pages` → `{pages:[…], limits:{canWhitelabel,canCustomCss,canCustomDomain,canMultiPage}, maxAssetBytes, maxCssBytes, defaultAccent}`; each page carries `resolved`, the exact object the public page renders with, so the admin preview cannot drift from the page it previews. `GET /api/admin/status-pages/slug-available?slug=` → `{slug,available,reason}` (admin; asked while the create dialog is typed in, so a taken name is known before the submit — it answers free/taken only, never which org holds it, because the slug namespace is global). `POST /api/admin/status-pages` (`{name,slug,visibility}`), `PATCH /api/admin/status-pages/:id` (`{name?,slug?,published?,visibility?,accent?,theme?,description?,supportUrl?,legalUrl?,hidePowered?,customCss?,componentIds?}`), `DELETE …/:id` (never the default page), `POST …/:id/rotate-token`. **Components:** `componentIds` is a positive subset; NO rows means *all*, so a page that shows everything keeps doing so as components are added. An incident appears on a page when it touches one of its components — or names none at all, which is how an org-wide announcement is expressed. **Private pages:** `visibility:'private'` mints an unguessable `access_token`; the page answers only for `?k=<token>` (then remembered in an httpOnly cookie so the links on the page work without carrying the secret), sends `noindex` in markup and header, and **404s rather than 403s** — a 403 would confirm that a page with that slug exists. Rotating the token is the revoke button. **Assets:** `PUT|DELETE /api/admin/status-pages/:id/asset/:kind` (`logo`|`favicon`, body `{data}` base64 or a `data:` URI, ≤512 KB — the string length bound is DERIVED from that byte limit; it was left at `isStr`'s 500-character default, so the route refused every image over ~375 bytes and the upload never worked for a real logo). Typed by **magic bytes, not by the declared type**, and SVG is refused outright: `/status/logo` can be opened directly and an SVG there would execute on the status page's own origin. Public: `GET /status[/:slug]/logo|favicon` — same-origin (no CSP `img-src` entry needed), `nosniff` + `Content-Disposition: inline`, ETag/304, 404 while unpublished or private-without-secret. An unset favicon falls back to the logo. **Custom CSS:** appended to the page's own `<style>` block, ≤20 KB, and rejected (never stripped) when it contains a closing `</style>` — that one string would turn a stylesheet field into stored XSS. **Custom domains:** `POST /api/admin/status-pages/:id/domain` `{domain}` stores it UNVERIFIED and returns the two DNS records to create; `POST …/domain/verify` looks for the TXT challenge at `_opscat-challenge.<domain>` (records are joined before comparison — a value may be split at 255 bytes) and only then sets `domain_verified_at`; `DELETE …/domain` clears it. The CNAME that routes traffic is deliberately NOT the proof: a dangling CNAME from a former owner would otherwise hand the name to whoever asks next. Our own hostnames (`baseUrl`, `gridHost`) cannot be claimed. `GET /api/public/tls-check?domain=` is Caddy's on-demand-TLS gate and answers 200 **only** for a stored, verified, plan-covered domain — answering broadly would make the deployment an open certificate mint and burn the ACME rate limit. On its own domain the page serves at that host's ROOT (`/`, `/summary.json`, `/feed.xml`, `/logo`, `/favicon`, `/subscribe`, `/report`, `/confirm`, `/unsubscribe`) and every link it prints stays on that host.
- `GET /api/synthetics/locations` (own + visible managed incl. `booked`/`region`/`nodeStatus`; POST creates a self-hosted sensor agent → `{probeKey}` once), `POST /api/synthetics/locations/provision` (BYO-cloud: provisions into the org's own AWS/GCP account), `POST|DELETE /api/synthetics/locations/:id/book` (managed-location booking, plan quota `managedLocations`, premium = Enterprise plan), `GET|POST|DELETE /api/synthetics/cloud-credentials` (encrypted at rest, responses carry label+hint only), `GET /api/synthetics/provider-catalog`, `GET/POST/PATCH/DELETE /api/synthetics/checks` (types http|icmp|dns|tcp|traceroute; **`target` is validated on create AND on edit** — it has to be something the runner can resolve to a host, so a bare `https://`, a target with a space or a tcp port outside 1-65535 is a 400 naming what to type instead, rather than a live check that fails every interval with `invalid url` and reads as an endpoint being down. Syntax only: a single-label host (`core-switch-01`) is accepted, and whether an address may actually be probed stays with `assertPublicHost` at run time; http checks accept `assertions {status?, keyword?, jsonPath?, jsonValue?}` and record `certDaysLeft` for https — ≤14 days raises `tls_cert_expiring`. Reputation assets are not in this table at all — they have their own tables and API at `/api/reputation/*`), `GET /api/synthetics/results` (latest per check×location), `GET /api/synthetics/results/series?checkId=&locationId=&hours=`, `GET /api/synthetics/history?minutes=&buckets=` (bucketed uptime per check for the HeatBar — worst-status-wins per bucket: `ok`/`warn`/`bad`/`na`, plus `uptimePct` and avg latency per bucket), `GET /api/synthetics/results/route?locationId=`, `POST /api/synthetics/run`
- **On-Call** (docs/ONCALL-V1.md slices 1-2). Two gates, deliberately different: **configuration** (teams, schedules, rotations, policies) is lead+, because it decides who gets woken at 03:00; **operation** (raise / acknowledge / cancel an alert, and a person's own contact methods) is open to every member, because the human the alert reached is often the most junior on the rota and an ack they cannot perform is an escalation nobody can stop.
  - `GET /api/oncall/now` → `{at, schedules:[{scheduleId,name,timezone,teamId,user:{id,name,email,color}|null,via,configured}]}` — every schedule resolved for ONE instant. `user: null` with `configured: true` is a **gap** (somebody should be on call and nobody is); `configured: false` means the schedule has no participants and no overrides yet, which is not the same thing.
  - `GET/POST /api/oncall/teams`, `PATCH/DELETE /api/oncall/teams/:id` — `{name, members:[userId]}`. A team is a name and a member list: `members` REPLACES the current list wholesale, and a user outside the org is ignored. Deleting a team leaves its schedules alive (`team_id` → NULL).
  - `GET/POST /api/oncall/schedules`, `GET/PATCH/DELETE /api/oncall/schedules/:id` — `{name, timezone, teamId}`. `timezone` is an IANA name, **validated on write** so a bad zone can never reach the resolver. Each schedule carries its resolved `onCall`, `via` (`'override'` | `'layer:<position>'` | null), `gapSince`, `layers[]` and upcoming `overrides[]`.
  - `PUT /api/oncall/schedules/:id/layers` — `{layers:[{rotation:'daily'|'weekly'|'custom', intervalD, handoffAt, participants:[userId], restrict:{days:[1..7],from:'HH:MM',to:'HH:MM'}|null}]}`. Replaces the whole set (≤5 layers, ≤50 participants each): layers are ordered and interdependent — the LAST one with somebody available wins — so patching one at a time would let a client leave the ladder in a state the server never validated. `handoffAt`'s **local time-of-day** in the schedule's zone is the handoff time. A participant outside the org is a 400, never a silent drop: a rotation short by one is invisible.
  - `POST /api/oncall/schedules/:id/overrides` `{userId, startsAt, endsAt}`, `DELETE /api/oncall/overrides/:id` — an override beats every layer for its window; the most recently created one wins an overlap.
  - `GET /api/oncall/schedules/:id/timeline?from=&days=` → `{shifts:[{startsAt,endsAt,userId,user,via}]}` (≤60 days) — coalesced runs, sampled from the resolver rather than solved, so it cannot disagree with `/now`.
  - A schedule that resolves to nobody raises the pipeline event **`oncall_gap`** (severity 70, device = schedule name), once per episode and re-armed when covered again.
  - `GET/POST /api/oncall/policies`, `PATCH/DELETE /api/oncall/policies/:id` — `{name, repeatN (0-5), highMin, hours:{days,from,to,tz}|null, steps:[{timeoutM, targets:[{kind:'user'|'schedule'|'team', refId}]}]}`. A policy is a **ladder, not a graph**: ordered rungs, one timeout each, an optional repeat, no branches and no conditions. `steps` replaces the whole set (≤10 steps, ≤20 targets each) for the same reason layers do. Every response carries `worstCaseMinutes` = `Σ timeoutM × (repeatN + 1)` — computed server-side so the editor cannot show a second answer. A target outside the org is a 400.
  - `GET /api/oncall/alerts?status=live|active|acked|exhausted|canceled|resolved|all&limit=` and `GET /api/oncall/alerts/:id` (adds `attempts[]` — who was notified, on which rung, how they were resolved). `nextAt` is when the current step escalates, null when nothing is ticking.
  - `POST /api/oncall/alerts` `{subjectKind:'case'|'incident', subjectId, policyId, urgency?, message?}` → 201, or **200 with the SAME alert** when one is already live for that subject+policy (idempotent — a flaring event must not ring the same person twice), or **202 `{suppressed:'maintenance'|'hours'}`**. A policy with no steps, an unknown subject and an already-closed subject are all 400: a ladder that cannot ring anybody is refused when it is wired up, not discovered at 03:00.
  - `POST /api/oncall/alerts/:id/ack` — any member. 409 when the alert is not `active`. `POST /api/oncall/alerts/:id/cancel` — stops the chain for the subject.
  - `POST /api/cases/:id/ack` — acknowledges the case AND whatever is escalating about it; 409 when it is already acknowledged and nothing is ringing. Writes `cases.acked_at`/`acked_by`, never `assigned_user_id`.
  - `GET /api/oncall/me` → `{methods[], rules[], fallbackEmail, shifts[]}`; `POST /api/oncall/me/methods` `{kind:'email'|'ntfy'|'telegram'|'pushover', address, label?}`, `DELETE /api/oncall/me/methods/:id`, `PUT /api/oncall/me/rules` `{rules:[{urgency:'high'|'low', delayM, methodId}]}` (whole set). **Every query is scoped to the caller** — a contact address is visible only to its owner. `sms`/`voice`/`push` answer 400 rather than being stored: their encryption story is slice 3, and a method that accepts an address and then sends nothing is the failure this module exists to prevent.
  - `GET /a/:token` renders a confirmation page, `POST /a/:token` performs the acknowledgement. Public and session-free — the token IS the credential (32 random bytes, SHA-256 at rest, single use, 24 h). It is two-step because mail scanners fetch every link in a message; a wrong, expired and already-used token get the same answer.
  - Alert rules gained `targetType:'channel'|'policy'` + `policyId` (`/api/rules`). A `policy` rule raises an alert instead of notifying a channel — this is what makes on-call work with zero flows. Every pre-existing rule keeps its behaviour, because `channel` is the default.
  - Pipeline events raised by the chain: **`alert_exhausted`** and **`alert_undeliverable`** (both severity 85). A `policy` rule may not target those or `oncall_gap` — the refusal is recorded in the notification log.
  - **Loud channels (slice 3).** `POST /api/oncall/me/methods` now accepts `sms` and `voice`; the number must be international format (`+4915112345678`) and is stored **encrypted**. `POST /api/oncall/me/methods/:id/verify` sends a six-digit code by SMS (hashed at rest, 15-minute TTL); `POST .../verify/confirm` `{code}` — a wrong code counts, five spend it (429), and an expired one is refused. An unverified sms/voice method is never used, and `GET /api/oncall/me` reports `blockedReason` in the same words the delivery log uses. `POST /api/oncall/me/methods/:id/test` sends a test through the REAL delivery path.
  - `GET /api/oncall/me/push-key` → the VAPID public key (stored once — regenerating it silently invalidates every browser). `POST /api/oncall/me/push` `{subscription, label}` registers a browser as a `push` contact method, encrypted at rest and deduped by endpoint. On iOS Safari a subscription cannot be created at all until the site is installed to the Home Screen.
  - `GET /v/:token/twiml` and `GET /v/:token/ncco` answer the provider's call flow — speak the alert, then gather one digit — and **do not acknowledge**: the provider fetches them before the phone rings. `POST /v/:token/digits` (Twilio, form) and `POST /v/:token/digits-json` (Vonage) acknowledge only on the digit `1`. Twilio's request signature is verified when that provider is configured.
  - `GET/PUT /api/admin/telephony` (admin) — `{provider:'twilio'|'vonage'|'webhook'|'', account, secret, from, webhookUrl, priceSmsMicros, priceVoiceMicros}`. The secret is stored encrypted and **never returned**; GET reports `hasSecret` and `configured`. A `webhook` gateway receives `POST {source, kind:'sms'|'voice', to, text, ackUrl}` through `safeFetch` and may answer `{id, price}`.
  - Plan features **`sms`** and **`voice`** from `pro` up — the only two in On-Call. Everything else, including the whole escalation chain, is ungated on every tier and in the community edition. The check happens at SEND time against the alert's org, because a contact method belongs to the person and the same human may be on call for two orgs.
  - `notifications.cost_micros` records what a metered message cost — from the provider when it reports one, from the org's configured per-message figure when it does not.
  - `GET /api/oncall/analytics?range=7d|30d|90d` (slice 5) → `{mtta:{avgMs,acked,unacked,daily[]}, escalation:{raised,escalated,acked,exhausted,rate,exhaustedRate}, perPerson[], perSchedule[], cost:{micros,messages}, truncated}`. **MTTA counts only cases that were actually acknowledged** — averaging the unacknowledged ones in as zero would make a team that answers nothing look fast, so they are reported separately as `unacked`. `perPerson` folds `alert_attempts` in **each person's own local time** (out of hours = before 08:00, from 18:00, or a weekend; a night = 22:00–06:00) using the same `localParts` the rotation engine uses; `timezone`/`tzSource` say which zone was used and where it came from (`user` → `schedule` → `utc-fallback`), because a figure computed in the wrong zone is worse than none. `truncated: true` means the per-person fold is a sample of the most recent 20 000 attempts, not a total.
...
- `GET /api/admin/agents` (`{id,name,group,hostname,platform,version,active,autoUpdate,lastSeenAt,online}`), POST (lead+, `{name,group,autoUpdate?}` default true) → `{token}` once — the name is unique **within the org** (`409` on a duplicate; it was globally unique until migration 036, which both leaked another tenant's names and made onboarding's fixed `my-first-server` claimable once per instance), PATCH `/:id` `{autoUpdate}`, `GET /api/admin/agents/:id/metrics?hours=`
