...
- `GET /api/assets` → unified list of monitored assets `[{kind:'agent'|'snmp'|'check'|'source', id, name, detail, status, lastSeen}]` — agents, SNMP targets, synthetic checks plus implicit log/event sources
