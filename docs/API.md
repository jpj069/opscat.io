...
read from `memberships` per request, never cached in the token. 19 tools (10 read, 9 write) — including `opscat_who_is_oncall`, which names any schedule that resolves to NOBODY separately in `gaps` so an agent does not have to notice a null in a list — each annotated and carrying an `outputSchema`; a read-only
...
- **On-Call** (docs/ONCALL-V1.md slice 1 — who has the duty; escalation policies and alerts are slice 2). Reads are open to the org, every write is lead+.
